...
# Define functions to be called upon interrupt, each writes to video_fifo
...
# Connect functions to GPIO interrupts, use bouncetime for debouncing
...
try:
    # We use the timeout argument to cause the script to end after 10s if the
    # quit button is never pressed (for perf)
    GPIO.wait_for_edge(27, GPIO.FALLING, timeout=10000)
    # We comment out the fifo write to prevent the script from waiting for 
    # something to consume the sent command
